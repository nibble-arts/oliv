...
//TODO load from configuration file
		$markup = array("'''" => "b","''" => "i","====" => "h3","===" => "h2","==" => "h1");

		foreach ($markup as $key=>$entry)
		{
			$o = "";
			
			$pattern = "/(?<prepart>.*)(?<mark>{$key})(?<innerpart>.*){$key}(?<postpart>.*)/";
			preg_match_all($pattern,$text,$matches,PREG_SET_ORDER);


//		preg_match_all("|(?<vor>.*)<(?<tag>[^>]+)>(?<inner>.*)</[^>]+>(?<hinter>.*)|U",
//    "Vordertext<b>Beispiel: </b>Mitteltext<div align=left>das ist ein Test</div>Hintertext",
...
//TODO loop over all matches
				$tag = $markup[$matches[0]['mark']];

				if ($matches[0]['mark'] == $key)
				{
//echoall($matches);

					$o .= $matches[0]['prepart'];
					$o .= "<$tag>" . $matches[0]['innerpart'] . "</$tag>";
